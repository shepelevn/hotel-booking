    ...
    <div class="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
        ...
        <div class="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
            <div class="bg-gray-50 w-full xl:w-96 flex justify-between items-center md:items-start px-4 py-6 md:p-6 xl:p-8 flex-col">
                ...
                <div class="flex flex-col md:flex-row xl:flex-col justify-between items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
                    <div class="w-full flex flex-col justify-start items-start flex-shrink-0">
                        ...
                        <div class="flex justify-center text-gray-800 md:justify-start items-center space-x-4 py-4 w-full">
                            ...
                            <p class="text-sm leading-5 ">{{ $booking->user->email }}</p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('bookings.destroy', ['booking' => $booking]) }}">
                        @csrf
                        @method('DELETE')
                        <button class="text-white bg-red-500 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-bold rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2" type="submit">{{ __('Cancel Booking') }}</button>
                    </form>
